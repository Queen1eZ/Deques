...
public abstract class BaseDequeTests extends BaseTest {
    ...
    */
    ...
    void newtest() {
        Deque<Integer> deque = createDeque();
        // Add elements that exceeds its initial capacity, making it use resize
        for (int i = 0; i < 100; i++) {
            deque.addFirst(i);
        }
        // Verify that removeLast works by removing elements in positive order
        for (int i = 0; i < 100; i++) {
            int removed = deque.removeLast();
            assertThat(removed).isEqualTo(i);
        }
